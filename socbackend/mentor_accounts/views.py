...
from .serializers import RegisterUserSerializer2, UserAutoCompleteSerializer2, UserProfileSerializer2
...
class RegisterUserView2(APIView):
    permission_classes = [AllowAny]  
    ...
    def post(self, request):
        ...
        if User.objects.filter(username=roll_number).exists():
            ...
            if UserProfile2.objects.filter(user=user).exists():
                user_profile = UserProfile2.objects.get(user=user)
        ...
        serializer = RegisterUserSerializer2(data=mutable_copy)
        ...
        if serializer.is_valid():
            ...
            if user_profile:
                mentor = Mentor.objects.create(user=user_profile)
                mentor.save()
            else:
...
class UserProfileView2(APIView):
    ...
    def get(self, request):
        user_profile = UserProfile2.objects.get(user=request.user)
        serializer = UserProfileSerializer2(user_profile)
...
class UserAutoCompleteView2(ListAPIView):
    """
    ListAPIView to access user data, for user searching
    """

    serializer_class = UserAutoCompleteSerializer2

    def get_queryset(self):
        queryset = UserProfile2.objects.all()
...
class CreateUserProfileView2(APIView):
    permission_classes = [IsAuthenticated]
